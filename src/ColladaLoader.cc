      ...
      const ignition::math::Matrix4d &_transform, Mesh *_mesh)
{
  ...
  if (!jointsXml)
  {
    ...
    ign_throw("Faild to parse skinning information in Collada file.");
  }
    ...
    std::map<unsigned int, unsigned int> &_duplicates)
{
  ...
  {
    ignerr << "<accessor> has no stride attribute in texture coordinate element "
          << "with id[" << _id << "]\n";
    return;
  }
          ...
          if (hasVertices)
          {
            ...
            if (_mesh->HasSkeleton())
            {
                  ...
                  i < skel->VertNodeWeightCount(daeVertIndex); ++i)
              {
                ...
                SkeletonNode *node =
                    _mesh->GetSkeleton()->NodeByName(node_weight.first);
